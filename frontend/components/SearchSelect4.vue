...
const props = defineProps(['modelValue', 'textValue', 'options', 'keyList', 'namaList', 'id', 'multiple', 'disabled', "apiUrl"]);
...
onMounted(async() => {
	...
	if(props.multiple == 'multiple'){
		...
		await loadDataFromApi(eli, props.textValue)
		if(eli){
	...
	if(el){
		...
		setTimeout(() => {
			...
			if(props.apiUrl != undefined){
				...
				console.log("add from options", props.id, props.options)
				eli.addOption(props.options)
			}
		}, 100);
	}
}
