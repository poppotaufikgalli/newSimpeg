...
const { data, pending, error, refresh } = await useFetch('/api/gets/group')

const doData = (id) => {
	const sid = $encodeBase64(id)
	navigateTo({ path: '/konfigurasi/group/'+sid })
}

const doDelete = (item) => {
	selId.value = item.id;
	modalStore.showModal("Hapus Data Group", "Apakah anda yakin untuk menghapus data Group an. "+item.nama+" ?")
}
...
const callback = async(e) => {
	...
	if(name == 'doAction'){
		await useFetch('/api/delete/group', {
			method: 'delete',
			body: JSON.stringify({
										...
										<template v-if="data" v-for="(item, idx) in data" :key="idx">
											<tr class="odd:bg-white even:bg-gray-100">
												...
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.nama}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.users.length || 0}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.lsakses}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top;">
