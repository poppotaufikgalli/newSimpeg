...
const { pending, data, refresh} = await useFetch('/api/gets/singkronisasi/bkn');
...
async function updateToken(type){
	...
	var result = await $fetch('/api/posts/singkronisasi_bkn/updateToken', {
		...
		body: JSON.stringify({
			type: type,
			ckey: data.value.ckey,
			csecret: data.value.csecret,
	...
	if(type == 'token_sso'){
		data.value.token_sso_expired = dayjs(result.token_sso_expired).format('YYYY-MM-DDTHH:mm:ssZ')
	}

	if(type == 'token_apimanager'){
		data.value.token_apimanager_expired = dayjs(result.token_apimanager_expired).format('YYYY-MM-DDTHH:mm:ssZ')
	}
