...
const {snip, bkn, jdiklat} = route.params

//const diklat01 = ref([])
//const diklat02 = ref([])
const status_bkn = ref('')

onMounted(() => {
	refreshNuxtData(["getdataDiklat", "getdataDiklatBkn"])
})

const { pending, data : diklat01, refresh} = await useLazyAsyncData('getdataDiklat', async() => {
	...
	var d1 = await $fetch('/api/posts/riwayat_diklat', {
		method: 'POST',
	...
	if(bkn == 1){
		if(jdiklat == 1){
			d1 = _filter(d1, {jdiklat: 1})
		}else{
			let notJ = _split("1,"+jdiklat, ',')
			d1 = d1.filter(o => {
				return !notJ.includes(o.jdiklat.toString())
			})
		}
  		
	}else{
		d1 = _filter(d1, {jdiklat: jdiklat*1})
	}
	return d1
});

const { pending: pending_bkn, data : diklat02, error} = await useLazyAsyncData('getdataDiklatBkn', async() => {
	console.log("CariData BKN")
	...
	console.log(path)
	if(bkn == 1){
		var result = await $fetch(`/api/gets/singkronisasi_bkn/getDataBKN/${path}/${nip}`)	
		return result.data
	}
	return null
});
...
const editData = (item) => {
	...
	let sjdiklat = $encodeBase64(jdiklat)
	navigateTo({ path: `/pegawai/${snip}/RiwayatPendidikan/diklat/form-${sjdiklat}-${stmulai}` })
}
		...
		<div class="mx-auto">
			...
			<div class="bg-white rounded-xl shadow py-4 px-6 border-t-2">
				<div class="flex justify-end mb-2">
					<button type="button" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" @click="editData()">Tambah Data</button>
				</div>
				<div class="mb-8">
							...
							<table class="min-w-full divide-y divide-gray-200">
								...
								<tbody class="divide-y divide-gray-200">
									...
									<template v-else-if="error">
										<tr>
											<td colspan="7" class="size-px px-1" style="vertical-align: top; min-width: 100px;">
												<span class="flex justify-center text-xs py-1 text-red-500">Get Data BKN Gagal :<b> {{error.data.data.message}}</b></span>
											</td>
										</tr>
									</template>
									<template v-else-if="diklat02" v-for="(item, idx) in diklat02" :key="idx">
										<tr class="odd:bg-white even:bg-gray-100">
											<td class="size-px px-1" style="vertical-align: top;">
												<button type="button" class="block" data-hs-overlay="#hs-ai-invoice-modal">
													<span class="flex justify-center text-xs px-3 py-0">{{idx+1}}</span>
											...
											<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
												<span class="flex justify-start text-xs py-0">{{item.jenisKursusSertifikat}}</span>
											</td>
											<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
												<span class="flex justify-start text-xs py-0">{{item.namaKursus}}</span>
											</td>
											<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
												<span class="flex justify-start text-xs py-0">{{item.noSertipikat}}</span>
											</td>
											<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
												<span class="flex justify-start text-xs py-0">{{item.institusiPenyelenggara}}</span>
											</td>
											<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
												<span class="flex justify-center gap-x-1 text-xs py-0">{{item.tanggalKursus}}<span>s/d</span>{{item.tanggalSelesaiKursus}}</span>
											</td>
											<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
