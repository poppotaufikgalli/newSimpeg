...
//const searchNip = ref('198602162008031001')
...
const { pending, data, refresh} = await useLazyAsyncData('getData', async() => {
	...
	return await $fetch('/api/posts/riwayat_stlud', {
		method: 'POST',
		body: body,
	});
});
...
const editData = (item) => {
	let skstlud = item ? $encodeBase64(item.kstlud) : $encodeBase64(null)
	let ststlud = item ? $encodeBase64(item.tstlud) : $encodeBase64(null)
	navigateTo({ path: `/pegawai/${snip}/RiwayatPegawai/stlud/${skstlud}-${ststlud}` })
}
...
const hapusData = async(item) => {
	...
	var {data, error} = await useFetch('/api/delete/riwayat_stlud', {
		method: 'DELETE',
		body: body,
	});
	...
	if(error.value){
		toast.add({
	    	id: 'error_delete_riwayat_stlud',
	    	description: error.value.data.data,
	    	timeout: 6000
	  	}) 	
	}else{
		toast.add({
	    	id: 'success_delete_riwayat_stlud',
										...
										<template v-if="data" v-for="(item, idx) in data" :key="idx">
											<tr class="odd:bg-white even:bg-gray-100">
												...
												<td class="size-px px-1 text-center" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.stlud?.nstlud}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.tstlud ? $dayjs(item.tstlud).format("DD-MM-YYYY").toString() : '-'}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.penyelenggara}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.tempat}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top;">
