...
const { pending, data, refresh} = await useLazyAsyncData('getData', async() => {
	...
	return await $fetch('/api/posts/riwayat_organisasi', {
		method: 'POST',
		body: body,
	});
});
...
const editData = (item) => {
	let snbahasa = item ? $encodeBase64(item.nbahasa) : $encodeBase64(null)
	navigateTo({ path: `/pegawai/${snip}/RiwayatPegawai/organisasi/${snbahasa}` })
}

const hapusData = async(item) => {
	var body = JSON.stringify({
		nip: item.nip,
		nbahasa: item.nbahasa,
	});

	var {data, error} = await useFetch('/api/delete/riwayat_organisasi', {
		method: 'DELETE',
		body: body,
	});
	...
	if(error.value){
		toast.add({
	    	id: 'error_delete_riwayat_organisasi',
	    	description: error.value.data.data,
	    	timeout: 6000
	  	}) 	
	}else{
		toast.add({
	    	id: 'success_delete_riwayat_organisasi',
	    	description: "Data Cuti berhasil di Hapus",
	    	timeout: 6000
	  	}) 	
	}
										...
										<template v-if="data" v-for="(item, idx) in data" :key="idx">
											<tr class="odd:bg-white even:bg-gray-100">
												...
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-start text-xs py-0">{{item.norg}}</span>
												</td>
												<td class="size-px px-1 text-center" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.jorg}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.jborg}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.tmulai ? $dayjs(item.tmulai).format("DD-MM-YYYY").toString() : '-'}}</span>
												</td>
												<td class="size-px px-1 text-center" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.takhir ? $dayjs(item.takhir).format("DD-MM-YYYY").toString() : '-'}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top;">
