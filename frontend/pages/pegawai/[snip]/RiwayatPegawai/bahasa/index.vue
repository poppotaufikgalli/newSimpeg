...
const { pending, data, refresh} = await useLazyAsyncData('getData', async() => {
	...
	return await $fetch('/api/posts/riwayat_bahasa', {
		method: 'POST',
		body: body,
	});
});
...
const editData = (item) => {
	...
	navigateTo({ path: `/pegawai/${snip}/RiwayatPegawai/bahasa/${snbahasa}` })
}
...
const hapusData = async(item) => {
	...
	var {data, error} = await useFetch('/api/delete/riwayat_bahasa', {
		method: 'DELETE',
		body: body,
	});
	...
	if(error.value){
		toast.add({
	    	id: 'error_delete_riwayat_bahasa',
	    	description: error.value.data.data,
	    	timeout: 6000
	  	}) 	
	}else{
		toast.add({
	    	id: 'success_delete_riwayat_bahasa',
	    	description: "Data Bahasa berhasil di Hapus",
	    	timeout: 6000
	  	}) 	
	}
										...
										<template v-if="data" v-for="(item, idx) in data" :key="idx">
											<tr class="odd:bg-white even:bg-gray-100">
												...
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-start text-xs py-0">{{item.nbahasa}}</span>
												</td>
												<td class="size-px px-1 text-center" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.kbahasa == 'A' ? 'Aktif' : 'Pasif'}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.jbahasa == 'D' ? 'Daerah' : 'Asing'}}</span>
												</td>
												<td class="size-px px-1 text-center" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.nilai}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top;">
