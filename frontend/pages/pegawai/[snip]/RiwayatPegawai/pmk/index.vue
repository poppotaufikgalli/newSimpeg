...
const { pending, data, refresh} = await useLazyAsyncData('getData', async() => {
	...
	return await $fetch('/api/posts/riwayat_pmk', {
		method: 'POST',
		body: body,
	});
});
...
const editData = (item) => {
	let stanggalAwal = item ? $encodeBase64(item.tanggalAwal) : $encodeBase64(null)
	navigateTo({ path: '/pegawai/'+snip+'/RiwayatPegawai/pmk/'+stanggalAwal })
}
...
const hapusData = async(item) => {
	...
	var {data, error} = await useFetch('/api/delete/riwayat_pmk', {
		method: 'DELETE',
		body: body,
	});
	...
	if(error.value){
		toast.add({
	    	id: 'error_delete_riwayat_pmk',
	    	description: error.value.data.data,
	    	timeout: 6000
	  	}) 	
	}else{
		toast.add({
	    	id: 'success_delete_riwayat_pmk',
	    	description: "Data Penyesuaian Masa Kerja berhasil di Hapus",
	    	timeout: 6000
	  	}) 	
	}
										...
										<template v-if="data" v-for="(item, idx) in data" :key="idx">
											<tr class="odd:bg-white even:bg-gray-100">
												...
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.ninstansi}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex gap-x-1 justify-center text-center text-xs py-0">{{item.tanggalAwal ? $dayjs(item.tanggalAwal).format("DD-MM-YYYY").toString() : '-'}}<span>s/d</span>{{item.tanggalAkhir ? $dayjs(item.tanggalAkhir).format("DD-MM-YYYY").toString() : '-'}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.masaKerjaTahun}} thn {{item.masaKerjaBulan}} bln</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top;">
