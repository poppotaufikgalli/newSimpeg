...
const { pending, data, refresh} = await useLazyAsyncData('getData', async() => {
	...
	return await $fetch('/api/posts/riwayat_tugas_tambahan', {
		method: 'POST',
		body: body,
	});
});
...
const editData = (item) => {
	...
	navigateTo({ path: `/pegawai/${snip}/RiwayatPegawai/tugas-tambahan/${stmtjab}` })
}
...
const hapusData = async(item) => {
	...
	var {data, error} = await useFetch('/api/delete/riwayat_tugas_tambahan', {
		method: 'DELETE',
		body: body,
	});
	...
	if(error.value){
		toast.add({
	    	id: 'error_delete_riwayat_tugas_tambahan',
	    	description: error.value.data.data,
	    	timeout: 6000
	  	}) 	
	}else{
		toast.add({
	    	id: 'success_delete_riwayat_tugas_tambahan',
	    	description: `Data Tugas Tambahan ${item.njab} berhasil di Hapus`,
	    	timeout: 6000
	  	}) 	
	}
										...
										<template v-if="data" v-for="(item, idx) in data" :key="idx">
											<tr class="odd:bg-white even:bg-gray-100">
												...
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-start text-xs py-0">{{item.njab}} - {{item.nunker}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
												...
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.status == 1 ? 'Aktif' : ''}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top;">
