...
const { pending, data, refresh} = await useLazyAsyncData('getData', async() => {
	...
	return await $fetch('/api/posts/riwayat_p2kp', {
		method: 'POST',
		body: body,
	});
});
...
const editData = (item) => {
	...
	let stselesai = item ? $encodeBase64(item.tselesai) : $encodeBase64(null)
	navigateTo({ path: `/pegawai/${snip}/RiwayatPegawai/skp/${sthnilai}-${stmulai}-${stselesai}` })
}
...
const hapusData = async(item) => {
	var body = JSON.stringify({
		...
		tmulai: item.tmulai,
		tselesai: item.tselesai,
	});

	var {data, error} = await useFetch('/api/delete/riwayat_p2kp', {
		method: 'DELETE',
		body: body,
	});
									...
									<tbody class="divide-y divide-gray-200">
										<template v-if="data" v-for="(item, idx) in data" :key="idx">
											<tr class="odd:bg-white even:bg-gray-100">
												...
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex gap-x-1 justify-center text-center text-xs py-0">{{item.tmulai ? $dayjs(item.tmulai).format("DD-MM-YYYY").toString() : '-'}}<span>s/d</span>{{item.tselesai ? $dayjs(item.tselesai).format("DD-MM-YYYY").toString() : '-'}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">
														{{item.hasil_kinerja}}-{{ratings[item.hasil_kinerja].nama}}
													</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">
														{{item.perilaku_kerja}}-{{ratings[item.perilaku_kerja].nama}}
													</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">
														{{item.kuadran_kinerja}}-{{kuadrankinerjas[item.perilaku_kerja].nama}}
													</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top;">
