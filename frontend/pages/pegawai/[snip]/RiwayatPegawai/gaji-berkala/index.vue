...
const { pending, data, refresh} = await useLazyAsyncData('getData', async() => {
	...
	return await $fetch('/api/posts/riwayat_gajiberkala', {
		method: 'POST',
		body: body,
	});
});
...
const editData = (item) => {
	let stmtngaj = item ? $encodeBase64(item.tmtngaj) : $encodeBase64(null)
	navigateTo({ path: '/pegawai/'+snip+'/RiwayatPegawai/gaji-berkala/'+stmtngaj })
}

const hapusData = async(item) => {
	var body = JSON.stringify({
		nip: item.nip,
		tmtngaj: item.tmtngaj,
	});

	var {data, error} = await useFetch('/api/delete/riwayat_gajiberkala', {
		method: 'DELETE',
		body: body,
	});
	...
	if(error.value){
		toast.add({
	    	id: 'error_delete_riwayat_gaji_berkala',
	    	description: error.value.data.data,
	    	timeout: 6000
	  	}) 	
	}else{
		toast.add({
	    	id: 'success_delete_riwayat_gaji_berkala',
	    	description: "Data Gaji Berkala berhasil di Hapus",
	    	timeout: 6000
	  	}) 	
	}
										...
										<template v-if="data" v-for="(item, idx) in data" :key="idx">
											<tr class="odd:bg-white even:bg-gray-100">
												...
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.tmtngaj ? $dayjs(item.tmtngaj).format("DD-MM-YYYY").toString() : '-'}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-xs py-0">{{item.mskerja}} thn {{item.mskerjabl}} bln</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.gpokkhir}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.nstahu}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
													<span class="flex justify-center text-center text-xs py-0">{{item.tstahu ? $dayjs(item.tstahu).format("DD-MM-YYYY").toString() : '-'}}</span>
												</td>
												<td class="size-px px-1" style="vertical-align: top; min-width: 100px;">
