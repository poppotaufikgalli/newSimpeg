...
const { pending, data, refresh} = await useLazyAsyncData('getData', async() => {
	...
	return await $fetch('/api/posts/riwayat_pemberhentian_jft', {
		method: 'POST',
		body: body,
	});
})
...
const editData = (item) => {
	...
	navigateTo({ path: `/pegawai/${snip}/RiwayatPegawai/pemberhentian-jft/${snbahasa}` })
}
...
const hapusData = async(item) => {
	...
	var {data, error} = await useFetch('/api/delete/riwayat_pemberhentian_jft', {
		method: 'DELETE',
		body: body,
	});
	...
	if(error.value){
		toast.add({
	    	id: 'error_delete_riwayat_pemberhentian_jft',
	    	description: error.value.data.data,
	    	timeout: 6000
	  	}) 	
	}else{
		toast.add({
	    	id: 'success_delete_riwayat_pemberhentian_jft',
	    	description: "Data Pemberhentian JFT berhasil di Hapus",
	    	timeout: 6000
	  	}) 	
	}
